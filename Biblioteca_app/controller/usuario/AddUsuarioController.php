...
try{
    ...
    && !is_null($_POST['cargo']) && !is_null($_POST['token'])){
        ...
        $cargo = $_POST['cargo'];
        $token = $_POST['token'];
        $response = UsuarioModel::AddUsuarios($documento,$nombre,$apellido,$email,$telefono,$password,$cargo,$token);
        if($response){
            Core::alert('Correcto','Se ha registrado el usuario correctamente','./usuarios');
        }else{
            Core::alert('Error','No se registro el usuario','./usuarios');
        }
    }else{
